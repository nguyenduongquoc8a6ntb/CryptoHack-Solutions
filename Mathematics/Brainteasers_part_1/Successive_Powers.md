...
- Do mọi số nguyên tố $p$ với ($p \neq 2$) luôn có 2 dạng $p \bmod 4 = 3$ và $p \bmod 4 = 1$ ta sẽ phân tích từng trường hợp để tìm ra $x$:
  - *Trường hợp 1:* $p \bmod 4 = 3$
    - Ta cần tìm $r$ sao cho $x^2 \equiv (a^r)^2 \equiv a \pmod p$.
      ...
      > Thay vào phương trình ban đầu: $x \equiv \pm a^{(p+1)/4} \iff x_1 \equiv a^{(p+1)/4} \pmod p$ và $x_2 \equiv -x_1 + p \pmod p$.
      
    - Vậy ta tìm được 2 nghiệm $x$: $x_1 \equiv a^{(p+1)/4} \pmod p$ và $x_2 \equiv p - x_1 \pmod p$ ($-x_1 \equiv p-x_1 \pmod p = p-x_1$).
